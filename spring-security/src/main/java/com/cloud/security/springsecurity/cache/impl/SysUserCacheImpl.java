package com.cloud.security.springsecurity.cache.impl;
...
import com.cloud.security.springsecurity.cache.ISysUserCache;
...
public class SysUserCacheImpl extends BaseServiceImpl<SysUser> implements ISysUserCache {
    ...
    public List<SysUser> cacheSelectList(SysUser query, String... emptyErrMsg) {
        ...
        dbValues.forEach(e -> {
            String entityKey = CLS_NAME.concat(":").concat(String.valueOf(e.getUId()));
    ...
    public List<SysUser> cacheSelectBatchIds(Collection<? extends Serializable> list, String... emptyErrMsg) {
        ...
        if(!CollectionUtils.isEmpty(queryKeys)){
            ...
            if(Objects.nonNull(cacheValues)){
                List<SysUser> cacheEntitys = JSONArray.parseArray(JSON.toJSONString(cacheValues), SysUser.class);
                if(queryKeys.size() == cacheEntitys.size()){
                    return cacheEntitys;
                } else {
                    List<String> cacheKeys = cacheEntitys.stream()
                        .map(e -> CLS_NAME.concat(":").concat(String.valueOf(e.getUId())))
                        .collect(Collectors.toList());
                    queryKeys.removeAll(cacheKeys);
                    List<SysUser> dbValues = super.selectBatchIds(queryKeys, emptyErrMsg);
                    if(!CollectionUtils.isEmpty(dbValues)){
                ...
                List<SysUser> dbValues = super.selectBatchIds(list, emptyErrMsg);
                dbValues.forEach(e -> {
                    String entityKey = CLS_NAME.concat(":").concat(String.valueOf(e.getUId()));
                    redisTemplate.opsForValue().set(entityKey,e);
                    redisTemplate.expire(entityKey,DEFAULT_EXPIRE_TIMES,DEFAULT_EXPIRE_TIMEUNIT);
    ...
    public PageBean<SysUser> cacheSelectPage(SysUser query) {
        ...
        PageBean<SysUser> cachePageBean = (PageBean<SysUser>)redisTemplate.opsForValue().get(pageBeanKey);
        ...
        List<Object> cacheValues = redisTemplate.opsForValue().multiGet(cahcePageKeys);
            ...
            List<SysUser> list = JSONArray.parseArray(JSON.toJSONString(cacheValues), SysUser.class);
